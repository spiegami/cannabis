...
export default class Argomento extends Component {
  ...
  render() {
    ...
    const display_theorem = (quale) => {
      if (this.state.cannabis[tema][argomento + quale]) {
        let color;
        switch(quale) {
          case "_SI":
            color = "#F1F8E9"
        ...
        return jsonToMaterial(this.state.cannabis[tema][argomento + quale], color)
      }
    }
      ...
      if (this.state.cannabis[tema][argomento + quale]) {
        if (quale==="") {return (
          <h4 style={{color: lightBaseTheme.palette.accent3Color}}>
            Spiegami la proposta di liberalizzazione della Cannabis
